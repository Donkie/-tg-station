...
/datum/plant_gene/trait/battery/on_attackby(obj/item/food/grown/G, obj/item/I, mob/user)
	if(istype(I, /obj/item/stack/cable_coil))
		...
		if(C.use(5))
			...
			pocell.maxcharge = G.seed.potency * 20e3

			// The secret of potato supercells!
			var/datum/plant_gene/trait/cell_charge/CG = G.seed.get_gene(/datum/plant_gene/trait/cell_charge)
			if(CG) // Cell charge max is now 40MJ (Same as bluespace power cells)
				pocell.maxcharge *= CG.rate * 100
			pocell.charge = pocell.maxcharge
			pocell.name = "[G.name] battery"
			pocell.desc = "A rechargeable plant-based power cell. This one has a rating of [siunit(pocell.maxcharge, "J", 1)], and you should not swallow it."
