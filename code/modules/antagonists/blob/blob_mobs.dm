	...
	if(..())
		...
		if(!factory)
			...
			if(locate(/obj/structure/blob/core) in blobs_in_area)
				adjustHealth(-maxHealth*0.05*SSMOBS_DT)
				var/obj/effect/temp_visual/heal/H = new /obj/effect/temp_visual/heal(get_turf(src)) //hello yes you are being healed
				if(overmind)
					H.color = overmind.blobstrain.complementary_color
				else
			...
			if(locate(/obj/structure/blob/node) in blobs_in_area)
				adjustHealth(-maxHealth*0.025*SSMOBS_DT)
				...
				if(overmind)
					...
					H.color = "#000000"
		if(damagesources > 0)
			adjustHealth(maxHealth*damagesources*0.0125*SSMOBS_DT) //take 1.25% of max health as damage per second when not near the blob or if the naut has no factory, 2.5% if both
