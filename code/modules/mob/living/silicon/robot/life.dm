...
/mob/living/silicon/robot/proc/use_energy()
	if(cell?.charge)
		if(cell.charge <= 100e3)
			uneq_all()
		var/amt = clamp(lamp_enabled * lamp_intensity * SSMOBS_DT * 1e3, 1e3, cell.charge)  // Lamp will use max 5kW, depending on brightness of lamp. If lamp is off, borg systems consume 1kW, or the rest of the cell if it's lower than that.
		cell.use(amt)
	else
		uneq_all()
		low_power_mode = TRUE
