 ...
 *
 *  auto_use_power(delta_time)            'game/machinery/machine.dm'
 *     Called by the SSmachines subsystem every cycle to automatically drain energy from
 *     the grid based on the active_power_usage and idle_power_usage settings, and which
 *     use_power mode the machine is in.
 *
 *     Return Value:
...
/obj/machinery/emp_act(severity)
	. = ..()
	if(use_power != NO_POWER_USE && !machine_stat && !(. & EMP_PROTECT_SELF))
